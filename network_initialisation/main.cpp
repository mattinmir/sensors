...
int main()
{
	...
	for (unsigned int i = 0; i < logfiles.size(); i++) 
	{
		...
		threads.push_back(thread(update_last_seen, ref(infiles.back()), ref(last_seen), ref(failures))); // update last seen for every logfile
	}
	...
	while (true)
	{
		...
		for (unsigned int i = 0; i < new_logfiles.size(); i++)
		{
			...
			if (!(find(opened_logfiles.begin(), opened_logfiles.end(), new_logfiles[i]) != opened_logfiles.end()))
			{
				...
				thread(update_last_seen, ref(infiles.back()), ref(last_seen), ref(failures)).detach(); // update last seen
